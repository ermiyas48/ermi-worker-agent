...
<style>
body{font-family:system-ui,sans-serif;max-width:640px;margin:2rem auto;padding:0 1rem;background:#fafafa;color:#111}
...
button{background:#111;color:#fff;border:0;border-radius:8px;padding:.65rem 1.1rem;cursor:pointer;font-size:1rem}
button.secondary{background:#fff;color:#111;border:1px solid #ccc;margin-left:.5rem}
.note{font-size:.85rem;color:#666;margin-top:1rem}
ol{padding-left:1.2rem}li{margin:.35rem 0}
</style></head><body>
<h1>ERMI Worker – First-time setup</h1>
<p>Sign in to ChatGPT once in the server Chromium profile. Credentials are never requested by this app.</p>
<div class="card"><div id="status" class="status wait">Checking…</div>
<div id="detail" style="margin-top:.75rem;font-size:.95rem"></div>
<div style="margin-top:1rem"><button id="btnStart">Start Browser</button>
<button id="btnCheck" class="secondary">Check Authentication</button></div></div>
<div class="card"><strong>Steps</strong>
<ol><li>Click Start Browser.</li><li>Complete ChatGPT login (incl. MFA) in the server Chromium.</li>
<li>Click Check Authentication.</li><li>Profile is saved; setup route disabled.</li></ol>
<p class="note">Never share the owner token.</p></div>
<script>
...
async function refresh(){try{const r=await fetch('/setup/status',{headers});const j=await r.json();
const el=document.getElementById('status');const d=document.getElementById('detail');
if(j.setupComplete){el.className='status ok';el.textContent='Setup complete';d.textContent='Profile preserved.';}
else if(j.authenticated){el.className='status ok';el.textContent='Authenticated';d.textContent=j.message||'';}
else{el.className='status wait';el.textContent='Waiting for sign-in';d.textContent=j.message||'';}
}catch(e){document.getElementById('status').className='status err';document.getElementById('status').textContent='Error';document.getElementById('detail').textContent=e.message;}}
document.getElementById('btnStart').onclick=async()=>{const r=await fetch('/setup/browser',{method:'POST',headers});const j=await r.json();document.getElementById('detail').textContent=j.message||j.error||JSON.stringify(j);refresh();};
document.getElementById('btnCheck').onclick=async()=>{const r=await fetch('/setup/status?detect=1',{headers});const j=await r.json();document.getElementById('detail').textContent=j.message||JSON.stringify(j);refresh();};
